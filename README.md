...
### 3.1.1 컨테이너 실행
컨테이너를 실행하고 쉘 환경으로 접근할 수 있는 명령어입니다.
...
> **설명:**
> - `docker run -it`: **인터랙티브 모드**로 컨테이너를 실행하여, 사용자와 상호작용할 수 있도록 합니다.
> - `--rm`: 컨테이너가 종료되면 자동으로 **삭제**되도록 합니다.
> - `pytorch/pytorch:latest`: 실행할 **도커 이미지**입니다.
> - `bash`: 컨테이너 내에서 **bash 쉘**을 실행하여, 터미널 환경에 들어가도록 합니다.

---

### 3.1.2 로컬 폴더 마운트
로컬 머신의 폴더를 **컨테이너 내부**에 마운트하여, 실시간으로 파일을 수정하고 테스트할 수 있게 합니다.
...
> - 이를 통해 **로컬에서 파일을 변경하면 컨테이너 내부에서도 실시간으로 반영**됩니다.
> - 컨테이너 내에서 `train.py`, `predict.py`, `app.py` 등을 수정하여 작업할 수 있습니다.
